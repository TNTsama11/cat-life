...
本仓库根目录包含多个子项目，游戏本体在 [`maosheng-xiuxian`](./maosheng-xiuxian) 目录下。

```
maosheng-xiuxian/
├── apps/web/                  # Web 游戏界面
├── packages/core/             # 核心引擎（状态、事件、突破、渡劫）
├── packages/data/             # 数据内容（事件、天赋、成就、境界）
├── packages/hooks/            # React hooks
└── packages/condition/        # 条件表达式引擎
```
...
所有人都可以提交内容。想加事件、天赋、成就，不需要改引擎，直接编辑 `maosheng-xiuxian/packages/data/src/content/` 下的数据文件即可。

### 目录结构

```
maosheng-xiuxian/packages/data/src/
├── content/
│   ├── events.ts               # 基础事件 + 特殊事件
...
`AGE` `CHR` `INT` `STR` `MNY` `SPR` `LIF` `PHASE` `REALM` `CULT` `SE` `TRIB` `APT` `COMP` `PHY` `FOR` `SPC` `HOME` `SEED` `DAO` `DEMON` `EXPO` `SEX` `ROM` `STER` `HAB` `TLT` `EVT` `TMS` `AEVT` `ATLT` `AACH`。
